...
- 本地自动化仅保留一个协调入口，每 30 分钟检查到期状态，串行执行每日论文、模型月检、会议周检。每日目标时间 21:30、模型每月 1 日 09:00、会议每周六 10:00（Asia/Shanghai）；开机后补最近到期且未完成的一期，不逐日重放。协调回执保存于忽略的 `build/reports/task-coordinator.json`；没有到期工作时静默退出。
