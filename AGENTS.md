...
- 修改前检查未提交改动，保留并排除与当前任务无关的文件。pull/fetch/push 失败时先自主检查权限、网络、代理、SSH、进程及远端状态，采用有超时的重试或已有 WSL 环境恢复；不能仅因首次失败就等待用户。真实冲突、无法无损整合的分叉或需破坏性操作时保留现场并说明。禁止强推、reset --hard、覆盖用户改动或擅自 stash；未确认同步成功前不修改业务文件。
...
- 本地自动化仅保留一个协调入口，按 Codex 自动化中实际保存的唤醒间隔检查到期状态，串行执行每日论文、模型月检、会议周检；提示词和文档不得另行硬编码唤醒间隔。每日目标时间 21:30、模型每月 1 日 09:00、会议每周六 10:00（Asia/Shanghai）；开机后补最近到期且未完成的一期，不逐日重放。协调回执保存于忽略的 `build/reports/task-coordinator.json`；没有到期工作时静默退出。
- 维护任务按阶段保存证据和进度，验证后分别提交并普通 push，确认远端后再执行下一项；不再留下定时任务自己的未发布提交阻塞 daily。会议每轮最多处理 20 篇正文总结，余量由 daily 的会议切片继续；已完成周检不因总结积压重复抓取。模型月检不等待会议积压。局部失败明确记录，不能冒充全部成功；基础设施故障有界重试并保留现场。
